...
The configuration below customizes Tab behavior to automatically complete when
there's only a single candidate, while cycling through multiple candidates when
available. Tab will wrap around from the last candidate back to the first.
Shift-Tab cycles backwards through the completion list.

Nerd-icons-corfu integration adds visual icons to completion candidates, making it
easier to distinguish between different types (functions, variables, classes, etc.)
at a glance. This uses the locally installed nerd-icons fonts without any network
requests, providing fast icon rendering in the completion popup.
...
#+begin_src emacs-lisp
      (use-package corfu
        ...
        (setq corfu-cycle t                ; Enable cycling for `corfu-next/previous'
              ...
              corfu-auto-delay 0.0         ; No delay - show completions immediately
              corfu-auto-prefix 1          ; Show completions after just 1 character
              ...
              corfu-max-width 100)         ; Limit popup width for faster rendering
        ...
        (defun mvh/corfu-complete-or-next ()
          "Complete if single candidate, otherwise cycle to next."
          (interactive)
          (if (= corfu--total 1)
              (corfu-complete)
            (corfu-next)))

        (define-key corfu-map (kbd "TAB") 'mvh/corfu-complete-or-next)
        (define-key corfu-map (kbd "[tab]") 'mvh/corfu-complete-or-next)
      ...
      ;; Nerd-icons-corfu adds nerd-icons to corfu completions
      (use-package nerd-icons-corfu
        :after (corfu nerd-icons)
        :demand t
        :config
        ;; Add nerd-icons formatter to corfu margin formatters
        (add-to-list 'corfu-margin-formatters #'nerd-icons-corfu-formatter)
        
        ;; Enable nerd-icons-corfu globally
        (setq nerd-icons-corfu-show-icon t))
...
#+begin_src emacs-lisp
  ...
  (use-package lsp-mode
    ...
    :config
    (setq lsp-idle-delay 0.1  ; Faster LSP response time
    ...
    :config
    ;; Function to update PATH with gem bin directories (duplicated here for availability)
...
and the default gem directory (for regular gem installs)."
    ...
    ;; Auto-install ruby-lsp and sorbet when needed
    (defun mvh/ensure-ruby-lsp-installed ()
      "Ensure ruby-lsp and sorbet are installed for the current Ruby.
This function checks if ruby-lsp is available and installs it if not.
For Sorbet projects, it also ensures the Sorbet gem is installed.
The function handles both system Ruby (with --user-install) and
chruby-managed Rubies, updating PATH as needed."
      ;; Update PATH first to ensure we check the right locations
      (mvh/update-gem-path-for-chruby)
      ;; First check in the current PATH (which includes chruby paths)
      (unless (executable-find "ruby-lsp")
        (message "ruby-lsp not found, attempting to install...")
        (let* ((ruby-cmd "ruby -e 'puts RUBY_VERSION; puts Gem.user_dir; puts Gem.default_dir'")
             (ruby-info (shell-command-to-string ruby-cmd))
             (ruby-lines (split-string ruby-info "\n" t))
             (ruby-version (car ruby-lines))
             (gem-user-dir (cadr ruby-lines))
             (gem-default-dir (caddr ruby-lines))
             (ruby-path (executable-find "ruby"))
        ...
        (message "Detected Ruby %s at %s" ruby-version ruby-path)
        (when use-user-install
          (message "System Ruby detected. Installing to user directory: %s" gem-user-dir))
        (let ((install-result (call-process-shell-command install-cmd nil nil nil)))
          (if (zerop install-result)
              (progn
                (message "ruby-lsp installation complete.")
                ;; Add gem bin directories to exec-path
                (when use-user-install
                  (let ((gem-bin-dir (expand-file-name "bin" gem-user-dir)))
                    (add-to-list 'exec-path gem-bin-dir)
                    (setenv "PATH" (concat gem-bin-dir ":" (getenv "PATH")))))
                ;; Also add the default gem bin directory for chruby-managed rubies
                (let ((default-gem-bin (expand-file-name "bin" gem-default-dir)))
                  (when (file-directory-p default-gem-bin)
                    (add-to-list 'exec-path default-gem-bin)
                    (setenv "PATH" (concat default-gem-bin ":" (getenv "PATH"))))))
            (message "Failed to install ruby-lsp. Please install manually: %s" install-cmd)))))

      ;; Check if this is a Sorbet project and ensure sorbet is installed
      (when (or (file-exists-p "sorbet/config")
                (locate-dominating-file default-directory "sorbet/config"))
        (unless (zerop (call-process-shell-command "gem list sorbet -i" nil nil nil))
          (message "Sorbet project detected but sorbet gem not found, installing...")
          (let ((install-cmd (if (string-prefix-p "/usr/bin" (or (executable-find "ruby") ""))
                                 "gem install --user-install sorbet sorbet-runtime"
                               "gem install sorbet sorbet-runtime")))
            (call-process-shell-command install-cmd nil nil nil)
            (mvh/update-gem-path-for-chruby)))))

    ;; Configure ruby-lsp for bundler projects
    (defun mvh/configure-ruby-lsp-for-project ()
      ...
      (when (fboundp 'lsp-mode)
        (let ((current-buffer (current-buffer))
          ...
          ;; Give chruby a moment to set up the environment
          (run-with-idle-timer
           0.1 nil
           (lambda ()
             (when (buffer-live-p current-buffer)
               (with-current-buffer current-buffer
                 ;; Ensure ruby-lsp is installed with the current Ruby
                 (mvh/ensure-ruby-lsp-installed)
                 ;; Check if we're in a bundler project
                 (let ((gemfile (locate-dominating-file current-dir "Gemfile")))
                   (when gemfile
                     ...
                     (let ((gemfile-path (expand-file-name "Gemfile" gemfile))
                       ...
                       ;; Set the buffer-local variable in the current Ruby buffer
                       (setq-local lsp-ruby-lsp-use-bundler has-ruby-lsp)))))))))

    ;; Hook to configure ruby-lsp when entering ruby-ts-mode
    ;; Add with lower priority (positive depth) to run after chruby
    (add-hook 'ruby-ts-mode-hook 'mvh/configure-ruby-lsp-for-project 90)
    (add-hook 'ruby-mode-hook 'mvh/configure-ruby-lsp-for-project 90)

    ;; Configure lsp-mode settings for ruby-lsp
    ...
    ;; Create the wrapper script at startup
    (let ((wrapper-path (mvh/create-ruby-lsp-wrapper)))
      ;; Add the wrapper directory to exec-path
      (add-to-list 'exec-path (file-name-directory wrapper-path))
      ;; Override the ruby-lsp server command to use our wrapper
      (with-eval-after-load 'lsp-mode
        (setq lsp-ruby-lsp-server-command (list wrapper-path))))))
...
#+begin_src emacs-lisp
  (use-package chruby
    ...
    :init
    ;; Initialize chruby paths
    (setq chruby-mode t)
    :config
    ;; Function to update PATH with gem bin directories
    (defun mvh/update-gem-path-for-chruby ()
      "Update PATH to include gem bin directories for the current Ruby.
This is necessary because chruby changes the Ruby executable but doesn't
automatically update Emacs' exec-path with the corresponding gem bin directories.
...
and the default gem directory (for regular gem installs)."
    ...
    ;; Auto-detect Ruby version from .ruby-version files
    (defun mvh/chruby-use-corresponding ()
      "Use chruby to activate the Ruby version specified in .ruby-version."
      (let ((ruby-version-file (locate-dominating-file default-directory ".ruby-version")))
        (when ruby-version-file
          (let ((ruby-version (with-temp-buffer
                                (insert-file-contents (expand-file-name ".ruby-version" ruby-version-file))
                                (string-trim (buffer-string)))))
            (when (and ruby-version (not (string-empty-p ruby-version)))
              (chruby ruby-version)
              ;; Update PATH to include gem bin directory
              (mvh/update-gem-path-for-chruby))))))

    ;; Ensure chruby is activated early in the mode hook
    (defun mvh/ruby-mode-chruby-setup ()
      "Setup chruby for Ruby mode, running early in the hook."
      (mvh/chruby-use-corresponding))

    ;; Add with high priority (negative depth) to run early
    (add-hook 'ruby-ts-mode-hook 'mvh/ruby-mode-chruby-setup -90)
