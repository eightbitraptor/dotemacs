...
autoloads and saves them to a file that loads quickly. We also ensure straight.el
packages can be found during compilation and set up automatic byte-compilation
of the tangled init.el file after all packages are loaded for better performance.

#+begin_src emacs-lisp
  ;; Enable package quickstart (must be near the beginning)
  (setq package-quickstart t)

  ;; Startup time reporting is handled in early-init.el

  ;; Ensure straight.el packages can be found
  (eval-when-compile
    (add-to-list 'load-path (expand-file-name "straight/build/" user-emacs-directory)))

  ;; Byte-compile init.el after all packages are loaded
  (add-hook 'after-init-hook
            (lambda ()
              (let ((init-el (expand-file-name "init.el" user-emacs-directory)))
                (when (and (file-exists-p init-el)
                           (not (file-exists-p (concat init-el "c"))))
