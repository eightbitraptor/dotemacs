...
#+begin_src emacs-lisp
      (use-package company
        ...
        (setq company-idle-delay 0.0              ; No delay - show completions immediately
...
chruby-managed Rubies, updating PATH as needed."
      (message "[ruby-lsp] ensure-ruby-lsp-installed called")
      (message "[ruby-lsp] Current Ruby: %s" (executable-find "ruby"))
      (message "[ruby-lsp] Current chruby: %s" (chruby-current))
      
      ;; First ensure we're using the project's Ruby version
      (let ((ruby-version-file (locate-dominating-file default-directory ".ruby-version")))
        (when ruby-version-file
          (let ((ruby-version (with-temp-buffer
            ...
            (message "[ruby-lsp] Project Ruby version: %s" ruby-version)
            (when (and ruby-version (not (string-empty-p ruby-version)))
              (chruby ruby-version)
              (message "[ruby-lsp] Switched to Ruby: %s" (executable-find "ruby"))))))
...
and the default gem directory (for regular gem installs)."
    ...
    (defun mvh/ruby-mode-chruby-setup ()
      ...
      (message "[chruby setup] Starting for buffer %s in %s" (buffer-name) default-directory)
      (message "[chruby setup] Ruby before: %s" (executable-find "ruby"))
      (mvh/chruby-use-corresponding)
      (message "[chruby setup] Ruby after: %s" (executable-find "ruby"))
      (message "[chruby setup] Current ruby: %s" (chruby-current)))
