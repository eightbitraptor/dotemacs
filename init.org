...
To make solaire mode work, we need to install a theme that supports it. I quite
like the nord theme, but the official one in the ~nord-theme~ package doesn't
support it so I'm using the ~doom-themes~ version.
